...
Requires `swaghub.command.vanish` (self) or `swaghub.command.vanish.others` (both default `op`). `swaghub.vanish.see` (default `op`), a **passive visibility** permission and not a command permission, lets a staff member see other vanished players.
