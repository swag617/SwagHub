...
Every `%swaghub_...%` token resolves internally — no PlaceholderAPI installation required to use them inside SwagHub's own menus/scoreboard/tablist/announcements/holograms. PlaceholderAPI is only needed to pull in *other* plugins' placeholders alongside them.
...
Requires `swaghub.command.proxy` (default `op`). Since a `GetServers` query is inherently asynchronous, this sends an immediate "querying" acknowledgement, then a follow-up message listing every proxy-reported server (including ones not in your local `proxy.servers` list — `GetServers` is proxy-wide) with its last-known count, once the response arrives.
...
- Every fetch has a 3-second timeout and is cached for 60 seconds; an unreachable, unconfigured, or slow server just yields no data — never blocks or throws.
