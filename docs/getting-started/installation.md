...
2. **No proxy-side plugin is required.** SwagHub's proxy service talks to BungeeCord entirely over the plugin-messaging channel `bungeecord:main` (registered under its legacy name `"BungeeCord"`). This is the same channel BungeeCord has always used.
